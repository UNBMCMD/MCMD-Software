...
const int TestingDataInterval = 15000 ; //15 seconds for Functional Healthy 
...
void Samples() {
  ...
  *p_current = ((*p_current) * 1.11 / 1000)-0.2;// + 1; //-0.2;//*200/1000; //Offset is 10 is still needed?
  SampleCount += 1;
...
void BatteryLevel() {
  ...
  /* if(HOWMANY==0){
     ...
     //Serial.println("Should be in Reduced");
     HOWMANY +=1;
    }
    else if(HOWMANY==1){
     chargelevelmAh = ShutdownThreshold - 100;
           //Serial.println("Should be in Shutdown");
     HOWMANY +=1;
    }
    else if(HOWMANY==2){
     chargelevelmAh=FunctionalThreshold - 100;
           HOWMANY +=1;
          // Serial.println("Should be back in reduced");
    }
    else{
     chargelevelmAh=FunctionalThreshold + 100;
          // Serial.println("Should be back in functional");
    }
    ...
    if (*pLowcount >= 60) { // && !report //If low for 60 samples (=60 seconds)
      ...
      *pReport = 1;
      ...
      if (DipSwitch3) { //Normal Mode
        ...
        TXdata2[TxValCount] = " 60 seconds of current below 0A  ";
      }
      TransmitNowFlag = 1;
      TxValCount += 1;
